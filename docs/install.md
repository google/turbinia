...
case. Processing Evidence directly from GCS requires that you use
[GCS FUSE](https://cloud.google.com/storage/docs/gcs-fuse) and mount your bucket
at a common path on each worker node. Once your GCS bucket is mounted, you can
...
*   [Create a new GCS bucket](https://cloud.google.com/storage/docs/creating-buckets)
    and create directories for `scripts` and `output/logs`
*   Mount your GCS bucket on your server instance using
...
*   ***NOTE:*** The `start-wrapper.sh` script has a
...
*   Upon start, your VM should mount your GCS Bucket, and copy the start scripts
    ...
    Turbinia worker
*   ***NOTE:*** If the GCS FUSE is used for log files, update the Turbinia
    config file with a new `LOG_FILE` path to write worker logs directly into
    GCS.
...
*   Choose one of the locations for storing the Turbinia config and configure
    ...
    *   `/etc/turbinia/turbinia.conf` **(RECOMMENDED)**
        ...
        *   `cp <localgitpath>/turbinia/config/turbinia_config.py
...
*   If you're doing GCP Turbinia, don't forget to return to
    [GCP Turbinia](#gcp-turbinia) and finish the rest of it
