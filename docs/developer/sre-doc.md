...
*   workstation only : Authenticate with the cloud-response project:
    *   `$ gcloud auth application-default login`
*   workstation only : set the correct GCP project
    *   `$ gcloud config set project cloud-response-prod`
*   workstation only : Add all GCE hosts to your ssh configuration
    *   `/google/bin/releases/cloudsystems/tools/ssh_config.par --project_id
        cloud-response-prod`
...
*   Check the server logs to see if it's waiting for any tasks to complete
    ...
    *   <code>$ ./update-gcp-infra.sh -i
    ...
    NOTE: If you are not running from your workstation you can remove the -s
...
*   If no tasks are open, shutdown the Turbinia infrastructure. The management
    ...
    *   <code>$ ./update-gcp-infra.sh -i
    ...
    NOTE: Workers are shut down asynchronously so it will take a while to show
    them as "RUNNING"
...
*   The management script will make sure startup is done in the correct order
    ...
    *   <code>$ ./update-gcp-infra.sh -i
    ...
    NOTE: workers are started up asynchronously so it will take a while to show
    them as "RUNNING"
...
*   Verify the logs of the new worker for correct instantiation.
    *   <code>$ ./update-gcp-infra.sh -i
    ...
    NOTE: if you are not running from your workstation you can remove
            the -s
...
![gcp_graph](../images/gcp_architecture.png) Google
