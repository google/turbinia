...
class BulkExtractorTask(TurbiniaTask):
  ...
  def generate_summary_report(self, output_file_path):
    ...
    Returns:
      list: formatted report output.
      str: summary of bulk extractor run.
    """
    report = []
    ...
    report.append(fmt.heading1('Bulk Extractor Results'))
    report.append(fmt.heading2('Run Summary'))
    report.append(
        fmt.bullet(
            'Program: {0} - {1}'.format(
    ...
    report.append(fmt.heading2('Scanner Results'))
    ...
    for f in xml_iter:
      if f.tag == 'feature_file':
        name = next(xml_iter)
        count = next(xml_iter)
        report.append(fmt.bullet('{0}:{1}'.format(name.text, count.text)))
        features_count += int(count.text)

    summary = '{0} hits have been identified.'.format(features_count)
